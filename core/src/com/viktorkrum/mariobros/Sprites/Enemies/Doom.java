 ...
 */
public class Doom extends Enemy
{
    ...
    public Doom(com.viktorkrum.mariobros.Screens.PlayScreen screen, float x, float y) {
        super(screen, x, y);
        frames = new Array<TextureRegion>();
        for(int i = 0; i < 4; i++)
            frames.add(new TextureRegion(screen.getAtlas().findRegion("doom1"), i * 68, 0, 68, 68));
        walkAnimation = new Animation(0.1f, frames);
        stateTime = 0;
        setBounds(getX(), getY(), 125 / MarioBros.PPM, 125 / MarioBros.PPM);
        setToDestroy = false;
        destroyed = false;
        angle = 0;
    ...
    public void update(float dt){
        ...
        if(setToDestroy && !destroyed){
            ...
            destroyed = true;
            setRegion(new TextureRegion(screen.getAtlas().findRegion("doom1"), 68, 0, 68, 68));
            stateTime = 0;
        }

        else if(!destroyed) {
            b2body.setLinearVelocity(new Vector2(-7, -1));
    ...
    protected void defineEnemy() {
        ...
        vertice[0] = new Vector2(-50, 50).scl(1 / MarioBros.PPM);
        vertice[1] = new Vector2(50, 50).scl(1 / MarioBros.PPM);
        vertice[2] = new Vector2(-50, -50).scl(1 / MarioBros.PPM);
        vertice[3] = new Vector2(50, -50).scl(1 / MarioBros.PPM);
        head.set(vertice);
